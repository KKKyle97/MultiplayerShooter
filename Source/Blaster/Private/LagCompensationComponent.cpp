	...
	const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	for (auto& HitLocation : HitLocations)
	{
		...
		if (World)
		{
			...
			ABlasterCharacter* ConfirmHitCharacter = Cast<ABlasterCharacter>(ConfirmHitResult.GetActor());
			if (ConfirmHitCharacter)
			{
				if (ConfirmHitResult.Component.IsValid())
				{
					UBoxComponent* Box = Cast<UBoxComponent>(ConfirmHitResult.Component);
				...
				if (ShotgunResult.HeadShots.Contains(ConfirmHitCharacter))
				{
					ShotgunResult.HeadShots[ConfirmHitCharacter]++;
				}
				else
				{
					ShotgunResult.HeadShots.Emplace(ConfirmHitCharacter, 1);
				}
			}
		}
	}
	...
	for (auto& HitLocation : HitLocations)
	{
		...
		if (World)
		{
			...
			if (ConfirmHitCharacter)
			{
				...
				if (ShotgunResult.BodyShots.Contains(ConfirmHitCharacter))
				{
					ShotgunResult.BodyShots[ConfirmHitCharacter]++;
				}
				else
				{
					ShotgunResult.BodyShots.Emplace(ConfirmHitCharacter, 1);
				}
			}
		}
	}
	...
	const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, const float HitTime)
{
	...
	PathParams.DrawDebugTime = EDrawDebugTrace::ForDuration;
...
FServerSideRewindResult ULagCompensationComponent::ServerSideRewind(ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart,
	const FVector_NetQuantize& TraceEnd, const float HitTime)
{
	const FFramePackage FrameToCheck = GetFrameToCheck(HitCharacter, HitTime);
	return ConfirmHit(FrameToCheck, HitCharacter, TraceStart, TraceEnd);
}
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	if (OldestHistoryTime == HitTime) // if hit time is exactly the last saved frame
		{
		FrameToCheck = History.GetTail()->GetValue();
		bShouldInterpolate = false;
		}
	else if (NewestHistoryTime <= HitTime) // if hit time is same or earlier than the newest saved frame
		{
	...
	while (Older->GetValue().Time > HitTime) // March back until : OlderTime < HitTime < YoungerTime
		{
                                                                  ...
                                                                  const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime, AWeapon* DamageCauser)
{
	...
	if (BlasterCharacter && HitCharacter && Confirm.bHitConfirmed && DamageCauser)
	{
		UGameplayStatics::ApplyDamage(HitCharacter,
			DamageCauser->FireDamage,
	...
	const TArray<FVector_NetQuantize>& HitLocations, const float HitTime)
{
	...
	{
		if (!HitCharacter || !HitCharacter->GetEquippedWeapon() || !BlasterCharacter)
			continue;

		float TotalDamage = 0;
		if (Confirm.HeadShots.Contains(HitCharacter))
		{
			float HeadShotDamage = Confirm.HeadShots[HitCharacter] * HitCharacter->GetEquippedWeapon()->FireDamage;
			TotalDamage += HeadShotDamage;
		}

		if (Confirm.BodyShots.Contains(HitCharacter))
		{
			float BodyShotDamage = Confirm.BodyShots[HitCharacter] * HitCharacter->GetEquippedWeapon()->FireDamage;
	...
	const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	if (BlasterCharacter && HitCharacter && Confirm.bHitConfirmed)
	{
		UGameplayStatics::ApplyDamage(
			HitCharacter,
			BlasterCharacter->GetEquippedWeapon()->FireDamage,
			BlasterCharacter->Controller,
...
{
	BlasterCharacter = !BlasterCharacter ? Cast<ABlasterCharacter>(GetOwner()) : BlasterCharacter;
	if (BlasterCharacter)
	{
