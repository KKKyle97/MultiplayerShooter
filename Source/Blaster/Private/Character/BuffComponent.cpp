...
{
	if (!bReplenishingShield || !BlasterCharacter || BlasterCharacter->IsElimmed()) return;
	...
	if (ShieldReplenishAmount <= 0 || BlasterCharacter->GetShield() >= BlasterCharacter->GetMaxShield())
	{
