...
{
	AShotgunWeapon* ShotgunWeapon = Cast<AShotgunWeapon>(EquippedWeapon);
	if (!ShotgunWeapon || !Character) return;
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	if (bScreenToWorld)
	{
		...
		HUDPackage.CrosshairColor = TraceHitResult.GetActor() && TraceHitResult.GetActor()->Implements<UBlasterDamagableInterface>() ? FLinearColor::Red : FLinearColor::White ;
	}
}
...
void UCombatComponent::SetHUDCrosshairs(float DeltaSeconds)
{
	...
	BlasterPlayerController = !BlasterPlayerController ? Cast<ABlasterPlayerController>(Character->Controller) : BlasterPlayerController;

	if (BlasterPlayerController)
	{
		BlasterHUD = !BlasterHUD ? Cast<ABlasterHUD>(BlasterPlayerController->GetHUD()) : BlasterHUD;
		if (BlasterHUD)
		{
			if (EquippedWeapon)
			{
			...
			CrosshairShootingFactor = FMath::FInterpTo(CrosshairShootingFactor, 0.f, DeltaSeconds, 40.f);
